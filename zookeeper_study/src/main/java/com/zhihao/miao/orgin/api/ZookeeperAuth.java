...
/**
 * Zookeeper 节点授权
 * 就是创建节点的时候指定一个认证在createConnection方法中：
 * 
 * zk = new ZooKeeper(connectString, sessionTimeout, this);
 * //添加节点授权
 * zk.addAuthInfo(authentication_type,correctAuthentication.getBytes());
 * 
 * 然后所有的方法比如create等操作都需要加入这个认证才能操作，当然我建立一个其他的不需要认证的连接也是可以的，这样的操作就不要认证了
 * 
 * main方法：
 ...
 *
 *	try {
 *			zk.create(PATH, "init content".getBytes(), acls, CreateMode.PERSISTENT);  //等等操作的时候就要加上这个认证了
 *		.....
 * 
 * 
 */
public class ZookeeperAuth implements Watcher {
	...
	/** 连接地址 */
	final static String CONNECT_ADDR = "192.168.5.124:2181,192.168.5.125:2181,192.168.5.126:2181";
	/** 测试路径 */
